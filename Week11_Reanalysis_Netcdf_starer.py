...
data_path = os.path.join('C:\\Users\\certain\\Desktop',
                         'X150.135.165.49.307.9.51.18.nc')

# Read in the dataset as an x-array
dataset = xr.open_dataset(data_path)  # The type of this object will be an xarray.DataArray
# look at it
dataset
...
dataset.values
dataset.dims
dataset.coords  # gives the coordinates like which latitude for each grid, and etc
...
timeslice = dataset["prate"].sel(
    time=slice(start_date, end_date))  # slice works more like %in% in R
