# This code is for week 9 forecast assignment of HWRS_501
# HAS_TOOLS class
...
month_week4_data = month_data[(month_data.index.day <= 30) &
...
month_week5_data = month_data[((month_data.index.day <= 6) &
                              (month_data.index.month == m)) |
                              ((month_data.index.day > 30) &
                              (month_data.index.month == m + 1))]
plot_title = month_name[m-1] + "'s Fifth Week"
ax[1].hist(month_data['00060_Mean'], bins=50, color='steelblue')
ax[1].set(xlabel='Flow (cfs)', ylabel='Frequency', title=plot_title)

fig.set_size_inches(8, 3.5)
fig.savefig("HW9_Plot_2.png")

Forecast_parm[0, 1] = stats.mode(month_week4_data['00060_Mean'])[0]
Forecast_parm[1, 1] = stats.mode(month_week5_data['00060_Mean'])[0]

# %%
# Generate Plot #3
# Scatterplot of this years flow vs last years flow for September
Last = data[(data.index.year == 2020) & (data.index.month == 9)]['00060_Mean']
Current = data[(data.index.year == 2021) & (data.index.month == 9)]\
          ['00060_Mean']
...
ax.scatter(data[(data.index.year == 2020) & (data.index.month == 9)]\
           ["00060_Mean"],
           data[(data.index.year == 2021) & (data.index.month == 9)]\
           ["00060_Mean"],
...
data_precip_daily = data_precip.resample('D').mean()
